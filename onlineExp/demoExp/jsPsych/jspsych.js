...
window.jsPsych = (function() {
  ...
  function TimelineNode(parameters, parent, relativeID) {
    ...
    var _construct = function() {
      ...
      // if there is no timeline parameter, then this node is a trial node
        ...
        } else if ((typeof jsPsych.plugins[trial_type] == 'undefined') && (trial_type.toString().replace(/\s/g,'') != "function(){return timeline.timelineVariable(varname);}")) {
  ...
  function evaluateTimelineVariables(trial){
    ...
    for (var i = 0; i < keys.length; i++) {
      ...
      if (typeof trial[keys[i]] == "function" && trial[keys[i]].toString().replace(/\s/g,'') == "function(){return timeline.timelineVariable(varname);}") {
